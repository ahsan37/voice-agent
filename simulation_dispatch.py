...
from src.agents.agent_service import initialize_agents, get_all_agents
...
if __name__ == "__main__":
    ...
    END_USER_SYSTEM_PROMPT = """# Appointment Scheduling Agent Prompt
    ...
    initialize_agents(END_USER_SYSTEM_PROMPT, 3, client)
    
    agents = get_all_agents()
    
    for agent in agents:
        logger.info(f"Generated Agent: {agent.id} - {agent.name}")
        logger.info(f"  Personality: {agent.personality}")
        logger.info(f"  Objective: {agent.system_prompt}")
